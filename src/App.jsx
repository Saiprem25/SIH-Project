        ...
        <div className="flex h-screen bg-[#f1f5f9] font-sans text-slate-800 overflow-hidden">
            ...
            <aside className="w-64 bg-[#1e293b] text-slate-300 flex flex-col shrink-0">
                <div className="h-20 flex items-center px-6 border-b border-slate-700/50">
                    <div className="w-8 h-8 bg-blue-600 rounded flex items-center justify-center mr-3 shadow-md">
                    ...
                    <div>
                        ...
                        <div className="text-[10px] text-slate-400">Secure Identities. Safer India.</div>
                    </div>
                </div>
                ...
                <nav className="flex-1 py-6 px-4 space-y-1 overflow-y-auto custom-scrollbar">
                    {navItems.map((item) => (
                        <button
                            ...
                            className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium transition-all ${activeTab === item.name ? 'bg-blue-600 text-white shadow-md' : 'hover:bg-slate-800 hover:text-white'
                                }`}
                        >
                            <svg className="w-5 h-5 opacity-80" fill="none" stroke="currentColor" viewBox="0 0 24 24">{item.icon}</svg>
                            {item.name}
                        </button>
                    ))}
                </nav>
            ...
            <main className="flex-1 flex flex-col h-full overflow-hidden relative">
                <header className="h-20 bg-white border-b border-slate-200 flex items-center justify-between px-8 shrink-0 z-30 shadow-sm">
                    <div className="relative w-96">
                        ...
                        <input type="text" placeholder="Search documents, ID numbers, or references..." className="w-full bg-slate-100 rounded-full py-2.5 pl-10 pr-4 text-sm focus:outline-none focus:ring-2 focus:ring-blue-600 border border-slate-200" />
                    </div>

                    <div className="flex items-center gap-6 relative">
                        {/* Animated Admin Dropdown */}
                        <div className="relative">
                            <div
                                className="flex items-center gap-3 cursor-pointer hover:bg-slate-50 p-1.5 rounded-lg transition-colors"
                                onClick={() => setIsAdminDropdownOpen(!isAdminDropdownOpen)}
                            >
                                <div className="w-10 h-10 rounded-full bg-blue-600 text-white font-bold flex items-center justify-center shadow">A</div>
                                <span className="text-sm font-semibold text-slate-700 select-none">Admin {isAdminDropdownOpen ? '▲' : '▼'}</span>
                            </div>

                            {isAdminDropdownOpen && (
                                <div className="absolute right-0 top-full mt-2 w-56 bg-white rounded-xl shadow-xl border border-slate-100 slide-down overflow-hidden z-50">
                                    <div className="px-4 py-3 border-b border-slate-100 bg-slate-50">
                                        <p className="text-sm font-bold text-[#1e293b]">Admin User</p>
                                        <p className="text-xs text-slate-500">admin.mha@gov.in</p>
                                    </div>
                                    <button onClick={() => { setActiveTab('Settings'); setIsAdminDropdownOpen(false); }} className="w-full text-left px-4 py-3 text-sm text-slate-600 hover:bg-blue-50 hover:text-blue-600 transition-colors">Workspace Settings</button>
                                    <button onClick={() => { setActiveTab('Users'); setIsAdminDropdownOpen(false); }} className="w-full text-left px-4 py-3 text-sm text-slate-600 hover:bg-blue-50 hover:text-blue-600 transition-colors">Manage Accounts</button>
                                    <div className="border-t border-slate-100">
                                        <button onClick={() => setIsAdminDropdownOpen(false)} className="w-full text-left px-4 py-3 text-sm text-red-600 hover:bg-red-50 transition-colors font-semibold">Sign Out Session</button>
                                    </div>
                                </div>
                            )}
                        </div>
